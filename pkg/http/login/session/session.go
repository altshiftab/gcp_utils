...
func PopulateBareEndpoints(bareEndpointsOverview *types.EndpointSpecificationOverview, sessionHandler SessionHandler) error {
	...
	refreshEndpointSpecification.HeaderParserConfiguration = &parsing.HeaderParserConfiguration{Parser: headerParser}
	refreshEndpointSpecification.Handler = func(request *http.Request, bytes []byte) (*muxResponse.Response, *muxResponseError.ResponseError) {
		ctx := request.Context()
	...
	endEndpointSpecification := bareEndpointsOverview.EndEndpoint
	if endEndpointSpecification == nil {
		return motmedelErrors.NewWithTrace(fmt.Errorf("%w (end)", muxErrors.ErrNilEndpointSpecification))
	}
	endEndpointSpecification.HeaderParserConfiguration = &parsing.HeaderParserConfiguration{Parser: headerParser}
	endEndpointSpecification.Handler = func(request *http.Request, bytes []byte) (*muxResponse.Response, *muxResponseError.ResponseError) {
		ctx := request.Context()
